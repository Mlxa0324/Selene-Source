...
| 二级菜单下键 | 焦点回到当前一级菜单项 |
| 禁用菜单 | 不展示「清晰度」和「内核」 |
| 底部弹框样式 | 背景必须保留视频可见度，不使用过实遮罩；一级菜单保持原字号和尺寸，二级菜单文字比一级菜单小 2 号，二级卡片按约 2/3 缩放；播放列表和播放线路属于内容卡片，不得套用一级菜单按钮尺寸；底部一级菜单行必须贴底固定，切换不同高度二级菜单时不得上下跳动；弹框顶部必须按当前二级菜单实际高度收紧，非选集菜单不得保留大块顶部空白 |
