...
纵向 Grid 需要支持焦点驱动的提前分页：电影、剧集、动漫、综艺分类页当前焦点进入末尾五行时，触发下一页后台预取，给慢接口保留加载缓冲。加载中、无更多数据或同一批数据已触发过时不能重复请求；新页数据追加后才允许下一次触发。电影、剧集、动漫、综艺分类页使用豆瓣推荐接口的 `page` 参数执行真实分页，并按 `source + id` 去重追加；播放历史和收藏夹当前服务层一次性返回完整列表，暂不伪造分页。
...
| 一级菜单 | 焦点移入即切换二级菜单内容，不需要按确认；左右切换时焦点必须留在一级菜单，只有上键才进入当前二级菜单 |
