...
### 2026-05-29 00:00:00
- 修复 TV 设置页保存服务器地址、账号和密码后仍停留在本地模式的问题：保存配置时现在会同步切回服务器模式，后续首页、搜索和详情链路可以立即读取这份共享账号配置，不会再出现“表单已保存但系统没吃到”的现象。
- TV 设置页改为完整遥控器友好布局：进入页面后首焦点会自动落到服务器地址，输入框改为“浏览态获焦、确认后编辑”，开关和滑杆统一支持左右键调节，纵向浏览不再被输入控件卡住；同时为“自动去广告 -> 弹幕服务器地址 -> 弹幕开关 -> 弹幕滑杆 -> 保存弹幕配置 -> 清除缓存”补齐显式上下焦点链，修复真机上焦点停在下半段不再继续下移的问题。另新增局域网手机扫码配置页，手机可扫码填写服务器、图片代理和弹幕地址并回填到 TV 表单。
- TV 返回键分发新增防重入收口：`TvBackHandler` 只响应首次按下，忽略长按重复事件，并在当前返回链路真正结束后才释放锁，避免从搜索、播放历史、收藏夹、设置等独立页返回首页时，同一次返回误继续触发首页退出确认弹框。
- TV 播放历史页与收藏夹页的标题区改为固定在顶部：页面标题和右侧“删除全部”按钮不再跟随纵向 Grid 一起滚走，长列表浏览时也能随时确认当前页面并快速清空。
- TV 详情页与 TV 全屏播放器主链路新增 M3U8 代理配置预热：进入页面时提前并行读取代理地址，起播、换源和换集不再每次重复等待配置读取；同时移除 TV 全屏初始化阶段的空跑地址解析，并跳过同地址无续播点的重复下发，优先保障首播与重复选集操作的响应速度。
- Bangumi 新番放送新增跨端页面兜底链路：`BangumiService` 在 `api.bgm.tv/calendar` 失败且无新鲜缓存时，会改抓 `bangumi.tv / bgm.tv / chii.in` 的 `calendar` 页面并解析整周番剧数据回写缓存，避免 TV 首页和普通端首页的新番分区因接口不可达而整块空白。
- Bangumi 新番放送主接口等待策略收紧到 5 秒：主接口超时后立即切到 `calendar` 页面备选方案，不再长时间卡在首选接口上，首页新番分区能更快回显兜底数据。

### 2026-05-28 00:00:00
- TV 横向/纵向影视列表新增滚动期封面延迟加载：复用 `TvVideoCard` 统一在快速滚动时先展示骨架、停稳后再发起封面请求，降低搜索页、首页、详情页等大列表同时刷图时的网络压力，并补齐对应 widget test 回归覆盖。
- TV 搜索页“影片推荐”改为被动推荐源：优先复用最近打开的两个详情页底部相关推荐分组，按最近访问顺序合并去重；当本地还没有沉淀相关推荐时，再回退到热门剧集前 10 条 + 热门综艺前 10 条作为兜底列表。

### 2026-05-27 00:00:00
- TV 端新增统一设计视口预设：支持 `TvDesignPreset.auto / hd720 / fullHd1080 / qhd1440`，低分辨率设备整体等比缩小并在新路由中继承当前预设，保持首页、详情页、搜索页、设置页、历史页与 TV 弹窗的视觉比例一致。
- 强制 TV 模式新增本地调试代码开关：`DeviceModeConfig.localDebugForceTvMode` 可直接把平板等设备切到 TV 入口，同时保留 `SELENE_FORCE_TV_MODE` 的命令行覆盖方式。
- 强制 TV 模式默认值改回关闭：开发包启动改为优先自动识别真实设备，手机和平板不再因 Debug 默认配置误进 TV 壳；只有显式传入 `SELENE_FORCE_TV_MODE=true` 时才强制进入 TV 模式。
- 设备类型识别新增“真实设备类型”链路：`AppDeviceService` 保留强制 TV 模式下的入口分流结果，同时新增忽略强制开关的物理设备识别，并按 600dp 最短边区分手机与平板，方便 TV 壳内继续识别当前是真 TV、平板还是手机。
- TV 首页在根级浏览态按 `Esc` 或返回键时新增退出确认弹框，先关闭筛选面板或回到顶部导航，只有再次返回并确认后才退出 IvyTV，降低遥控器误触直接离开应用的风险。
- TV 从详情页或播放历史页返回上一级后，会重新刷新首页继续观看数据，避免返回首页时展示旧缓存。
- TV 继续观看进入详情页后优先等待并播放保存线路；保存线路未命中时按总集数相同源兜底，仍未命中则随机选择可用线路播放。
- TV 全屏播放器底部进度条的当前时间与总时长改为固定时间槽位并启用等宽数字，修复长按快进/快退时进度轨道在左侧时间附近来回跳动的问题。
- TV 搜索页右侧搜索历史/热词最右列按右键保持当前焦点，不再跳出内容区；右侧内容向下浏览时自动纵向滚动，让获焦项尽量停留在屏幕中段。
- TV 详情页横向列表首尾焦点安全留白按海报获焦放大重新加大，修复相关推荐长按到右端再回到首项时焦点框贴边像被裁剪的问题。
- TV 全屏播放器播放线路和播放列表二级内容卡片改为更宽更高的展示尺寸，线路名、集数和选集长标题都完整留在卡片边框内。
- TV 全屏播放器底部菜单背景透明度提高，一级和二级菜单按钮统一收窄约三分之一，播放列表选集卡片支持完整多行标题并随标题增高。
- TV 全屏播放器「其它」菜单的片头/片尾入口改为短按确认、空格或 Enter 保存当前时间点，长按清空，并在按钮上方展示操作提醒。
- TV 详情页横向线路、选集、分组和相关推荐列表增加首尾焦点安全留白，避免首个卡片或按钮获焦后左侧描边被裁剪遮挡。
- TV 详情页与全屏播放器的线路列表新增集数尾标，例如 `最大资源（99）`，并按集数倒序展示，方便优先切换同集数更完整的资源。
- TV 全屏播放器左右键快进/快退步进调柔：前 5 秒固定 5 秒小步进，之后平滑加速到 19 秒封顶，降低长按初期跳动感。
- TV 详情页选集分组改为获焦即切换上方选集列表，不再需要按确认键，保持与全屏播放列表分组一致的遥控器操作手感。
