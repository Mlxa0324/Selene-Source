...
### 2026-04-09 00:00:00
- 调整 Android 息屏播放生命周期判定：进入后台时会额外记住“进入后台前原本正在播放”的意图，避免 WebView 被系统先暂停后，原生后台接管因播放态瞬时变为 `false` 而被短路。
- 补充 Android 息屏接管调试日志：新增生命周期决策、动作选择与后台接管早退原因日志，便于区分是未触发、被 PiP 抑制、还是因播放态/请求条件不满足而跳过。
...
### 2026-04-03 00:00:00
- 调整 Android 在线播放进入画中画时的内核切换时机：WebView 前台小窗阶段不再立即切到原生后台播放器。
- 新增 Android PiP WebView 延迟接管策略：仅在 PiP 离开可见阶段后超过 1 秒，才切换到息屏播放允许的原生后台播放链路。
- 保留现有 Android 前后台恢复链路不变，确保从后台回前台后仍能恢复到前台播放器。
