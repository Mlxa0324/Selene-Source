...
### 2026-05-17 00:00:00
- 应用设置中的“主题色”切换收敛为 `经典影院绿 / 奈飞红 / 清澈蓝` 三套干净固定方案，默认保持经典影院绿。
- 新的主题色选择会本地持久化保存，下次启动继续沿用，并兼容旧 `ocean_blue` 配置自动映射到清澈蓝。
- 首页、启动页、登录页、播放器与直播页浅色背景改为更接近白色的主题轻染渐变，减少原先偏深偏灰的观感。
- 选集、换源、下载、直播正在播放状态与设置页中的绿色强调元素统一跟随全局主题色，选中卡片改为白底轻染色与浅边框，避免脏灰色块。
