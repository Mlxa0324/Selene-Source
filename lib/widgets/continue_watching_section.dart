...
class _ContinueWatchingSectionState extends State<ContinueWatchingSection>
  ...
  List<PlayRecord> _filterPlayableRecords(List<PlayRecord> records) {
    // 继续观看列表不展示离线缓存记录
    return records.where((record) => record.source != 'local').toList();
  }
  ...
  Future<void> _loadPlayRecords() async {
    ...
    try {
      ...
      if (cachedRecordsRes.success && cachedRecordsRes.data != null) {
        final cachedRecords = _filterPlayableRecords(cachedRecordsRes.data!);
        // 有缓存数据，立即显示
        if (mounted) {
          setState(() {
    ...
    try {
      if (mounted) {
        ...
        if (cachedRecordsResult.success && cachedRecordsResult.data != null) {
          final cachedRecords =
              _filterPlayableRecords(cachedRecordsResult.data!);
          setState(() {
            _playRecords = cachedRecords;
          });
